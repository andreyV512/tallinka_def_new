...
typedef void(*_ChebyshevI)(double *data, int N, int order, double sampleRate,
	double cutoffFrequency, double centerFrequency, double widthFrequency,
	double rippleDb, int type);
...
// ! @param widthFrequency - ширина фильтра
// ! @param type - тип фильтра 0 - LowPass,1 - HighPass,2 - BandPass,3 - BandStop
typedef void(*_Butterworth)(double *data, int N, int order, double sampleRate,
	double cutoffFrequency, double centerFrequency, double widthFrequency,
	int type);
...
// ! @param type - тип фильтра 0 - LowPass,1 - HighPass,2 - BandPass,3 - BandStop
typedef void(*_Elliptic)(double *data, int N, int order, double sampleRate,
	double cutoffFrequency, double centerFrequency, double widthFrequency,
	double rippleDb, double rolloff, int type);
...
public:
	...
	public:
		...
		void Load(TIniFile* _ini);
		void Save(TIniFile* _ini);
		void Load(FILE* _df);
		void Save(FILE* _df);
	};
private:
...
public:
	// ! Конструктор
	Filters(TIniFile* _ini, AnsiString _type);
	// ! @brief Загружает настройки из файла
	// ! @param type "Pp" или "Pr"
	void LoadSettings(void);
	// ! Фильтрует полученный сигнал
	void toFilter(double *data, int N);
	inline bool IsFilter(void)
	{
		return(settings.isFilter);
    }
	void Save(FILE* _df);
};

extern Filters *CrossFilter;
extern Filters *LinearFilterIn;
extern Filters *LinearFilterOut;
