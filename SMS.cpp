...
SMS* sms;
//---------------------------------------------------------------------------
SMS::SMS(TComponent *Owner,HWND _main_handle, UINT _sms_msg)
{
...
String SMS::CreateServerRespond()
{
	...
	if (rec.SubString(1,5) == "zone ")
	{
		...
		a+=(LPARAM)((th*10000));
		TPr::pr(a);
		Post((WPARAM)zone,(LPARAM)(th*10000));
		return "zone ok";
	}
...
void SMS::Post(WPARAM _w, LPARAM _l)
{
	if (PostMessage(main_handle, sms_msg, _w, _l) == 0)
		FATAL("SMS::Post: не могу послать сообщение");
}
